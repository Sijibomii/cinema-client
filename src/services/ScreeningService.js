...
export const connect = () => {
  if (!_socket || _socket.closed) {
    ...
    _socket = webSocket(`ws://django-movies-backend.herokuapp.com/movie/?token=${token}`);
...
export const getScreenings = async () => {
  const url = `${process.env.REACT_APP_BASE_URL}/api/movie/screenings/all/`;
  try {
...
export const getScreeningByMovie = async (id) => {
  const url = `${process.env.REACT_APP_BASE_URL}/api/movie/screening/movie/${id}/`;
  try {
    const response = await axios.get(url);
    return { response, isError: false };
  } catch (response) {
...
export const getSeatsByScreening = async (id) => {
  const url = `${process.env.REACT_APP_BASE_URL}/api/seats/screening/${id}/`;
  try {
    const response = await axios.get(url);
    return { response, isError: false };
  } catch (response) {
...
export const getSeatsAssigned = async () => {
  const url = `${process.env.REACT_APP_BASE_URL}/api/seats/`;
  const token = getAccessToken();
...
export const getSeatsPay = async () => {
  const user=getUser()
  const url = `${process.env.REACT_APP_BASE_URL}/api/seats/screening/pay/${user.id}/`;
  const token = getAccessToken();
  const headers = { Authorization: `Bearer ${token}` };
  try {
...
export const getDownloadTicket = async (id) => {
  ...
  const url = `${process.env.REACT_APP_BASE_URL}/api/tickets/${id}/`;
  const token = getAccessToken();
  const headers = { Authorization: `Bearer ${token}` };
...
export const getSeatsBooked = async () => {
  const url = `${process.env.REACT_APP_BASE_URL}/api/seats/booked/`;
  const token = getAccessToken();
  const headers = { Authorization: `Bearer ${token}` };
  try {
    const response = await axios.get(url, { headers });
